...
# Используем resource_path для получения иконки
icon_path = resource_path("appicon.ico")
print(f"Icon path: {icon_path}")  # Отладочная информация
try:
...
def download_mp3(youtube_url):
    try:
        ...
        # File path in the Downloads folder
        output_file = os.path.join(downloads_folder, f"{video_title} [mp3].%(ext)s")
        ...
        error_label.config(text=f"Audio was successfully saved\nin downloads folder.", font=("Lucida Console", 11))
    except Exception as e:
